...
/* 
	Un CDI bootable pour Dreamcast de type Audio/Data est compos� de plusieurs parties (dans l'ordre) :
	
		- 352 800 octets de z�ros binaires (padding ?),
		- La piste audio (mode audio donc) de 302 secteurs (num�rot�s de 0 � 301) dans le cas d'un CDI avec la 
		  piste "audio.raw" du kit ECHELON.
		- Une piste "GAP" servant � d�limiter la piste audio de la piste de donn�es, sur 350408 bytes. Elle est de deux types
		  diff�rents d�taill�s ci-dessous.
		- Une piste de donn�es, en Mode 2 Form 1 (CD/XA), commencant au secteur (lba ou msinfo) 11702 (toujours pour audio.raw 
		  de ECHELON).
		- Quelques secteurs de padding (2 pour �tre exact)
	...
	il faut rajouter des secteurs de padding pour arriver � 302. Si c'est sup�rieur � 302, on fait rien. En effet, 302 secteurs 
	est la taille minimale d'une piste. Certains graveurs ont du mal � graver ce genre de pistes si c'est inf�rieur.
	
	Voici la structure du CDI en sch�ma :

	+=================================================+
	|         352 800 octets de z�ros binaires        |
	+=================================================+
	|     Piste audio : 302 secteurs min, 710 304     |
	+-------------------------------------------------+	
	 |            0x108 octets de z�ros              |
	 +-----------------------------------------------+
	 |  --- La piste elle m�me (705 600 octets)---   |
	 +-----------------------------------------------+
	 |              11760 octets de z�ros            |
	+=================================================+
	|                    piste GAP                    |
	+-------------------------------------------------+
	 |   1st piste : gap_dummy_sector 175 200 octets |
	 +-----------------------------------------------+
	 |   2nd piste : gap_dummy_sector2 : m�me taille |
	+=================================================+
	|  Donn�es : au secteur 11702, taille bloc : 2336 |
	+-------------------------------------------------+
	 |             2048 octets de donn�es            |
	 +-----------------------------------------------+
	 |                4 octets d'EDC                 |
	 +-----------------------------------------------+
	 |              172 octets d'ECC P               |
	...
	+=================================================+
	 ...
	 /////////////////////////////////////////////////
	  ...
	  |             Infos piste datas :             |
	  +---------------------------------------------+
	   |        M�me chose que la piste audio...   |
	  +---------------------------------------------+
	  |                 Header final                |
	  +---------------------------------------------+
	   |  m�me chose que les autres jusqu'au nom   |
	   +-------------------------------------------+
...
// Plus some more documentation from some project that no longer exists(?):
/*
...
#define MINIMUM_AUDIO_LBA 302

// d�but de l'image : 352 800 bytes de zero binaires
static const unsigned int cdi_start_file_header = 352800;

// --- AUDIO TRACK ---

// Commence uniquement 108h bytes  apr�s. Les 108h bytes avant de commencer la vraie piste audio sont des z�ros binaires.
static const int cdi_audio_start_offset = 0x108;

// --- GAP ---

// ce "gap" l� est pour le cas d'une image data/data. Ce type d'image commence par une piste GAP de 350 400 bytes (soit 150 secteurs).
static const int gap_data_start_image_count = 150;

// le "gap" est une piste qui s�pare la piste audio et la piste de datas.
...
// un secteur GAP 1 est constitu� de �a :
static const int gap_dummy_sector_size = 6;
static const uint32_t gap_dummy_sector[6][2] = {
		{0x002, 0x20}, 
		{0x006, 0x20}, 
		{0x91c, 0x3f}, 
		{0x91d, 0x13}, 
...
// un secteur GAP 2 est constitu� de �a :
static const int gap_dummy_sector_2_size = 101;
static const uint32_t gap_dummy_sector_2[101][2] = {
...
static const unsigned int sector1[28][2] = {
		{0x00000, 0x02}, {0x00002, 0x96}, {0x00006, 0x78}, {0x00007, 0xB0}, {0x00024, 0x0E}, {0x00025, 0xB1}, {0x00038, 0x02},
		{0x00041, 0x0E}, {0x00042, 0xB1}, {0x0005a, 0xff}, {0x0005b, 0xff}, {0x0005c, 0xff}, {0x0005d, 0xff}, {0x0005e, 0xff},
...
static const unsigned int sector2[31][2] = {
		{0x00000, 0x02}, {0x00002, 0x96}, {0x00006, 0x9c}, {0x00007, 0x04}, {0x00010, 0x02}, {0x00018, 0x01}, {0x00020, 0x00},
		{0x00021, 0xDD}, {0x00038, 0x01}, {0x0003c, 0x04}, {0x0005a, 0xff}, {0x0005b, 0xff}, {0x0005c, 0xff}, {0x0005d, 0xff}, 
		...
		{0x000b0, 0x02}, {0x000b8, 0x00}, {0x000b9, 0xDD},
};
